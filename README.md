...
| Suites                 | API, UI (real backend), UI with mocked failures, visual, accessibility, cross-browser smoke                                          | UI end-to-end, API contract (Zod), visual, accessibility (including keyboard navigation)                                                                                                                                         |
| Browsers               | Chromium for the whole suite; the `ui` smoke tests also run in Firefox and WebKit ([ADR-0016](docs/adr/0016-cross-browser-smoke.md)) | Chromium, Firefox and WebKit                                                                                                                                                                                                     |
...
**Status: API, UI, mocked-UI, visual, accessibility and cross-browser smoke suites (229 tests), tiered CI, a published report with trend history, flake handling, a PR coverage-gap flagger, and a weekly upstream pin-drift check are in place.** Nothing in the v1 scope is outstanding. The `ui-mocked` project (43 tests) covers the failure paths of every page; what it doesn't cover is listed in the [test strategy](docs/test-strategy.md).
...
A weekly check reports when a pin has fallen behind its upstream for long enough to look at, and
never bumps one ([ADR-0002](docs/adr/0002-pin-upstream-app-versions.md), amendment).
...
cp .env.example .env
npm install
npx playwright install chromium   # add firefox webkit to run the cross-browser smoke
npm run docker:up
```
...
| `npm run test:cross-browser` | the `ui` smoke tests in Firefox and WebKit          | (part of full)  |
| `npm run test:full`          | regression + a11y + visual + cross-browser          | nightly, manual |
...
tests/
  ...
  api/          API contract tests, no browser; every response is checked against the OpenAPI spec
  ui/           functional UI specs, real backend (the smoke tests also run in Firefox and WebKit)
...
tools/
  ...
  ci/            readiness wait, report generation and publishing, the tag check, and the pin-drift check
.github/workflows/  CI (tiers, publishing) and the weekly pin-drift check
...
tiers, the pinned visual image, the a11y ratchet, flake handling, API contract validation, the
coverage-gap flagger, cross-browser scope) is recorded in
